...
[pop, theta] = QDEsolset_sir(npop, m,n); % Generation of Population
...
for j=1:npop
 cpop = pop(j,:,k);
 neighborhood = [cpop(1) - step, cpop(2), cpop(3), cpop(4);
                 cpop(1) - step, cpop(2) - step, cpop(3), cpop(4);
                 cpop(1), cpop(2) - step, cpop(3), cpop(4);
                 cpop(1) + step, cpop(2) - step, cpop(3), cpop(4);
                 cpop(1) + step, cpop(2), cpop(3), cpop(4);
                 cpop(1) + step, cpop(2) + step, cpop(3), cpop(4);
                 cpop(1), cpop(2) + step, cpop(3), cpop(4);
                 cpop(1) - step, cpop(2) + step, cpop(3), cpop(4)];
 b = randperm(8,3);
 ...
 %dv = round(abs(r1 + F * (r2 - r3)));

dv = QDE_mutation(pop(:,:,k), F, r1, r2, r3);

% Quantum Rotation Update

%[theta(j,:)] = QDErot(theta(j,:), r1, r2, r3, dv, alpha);
%duv = round(QDEmeasure(theta(j,:), m, n, dv,pop));
       
%uv = QDEopticdecross_sir(pop(j,:), duv, cro, m, n);
